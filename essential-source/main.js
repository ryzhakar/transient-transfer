...
(function() {
    ...
    function copyLink(url) {
        const fullUrl = url.startsWith('http') ? url.replace('/get', '') + '?dl=1' : window.location.origin + '/get' + url;
     ...
     * For single file: direct link, for multiple: ZIP link
    ...
    function downloadAll() {
        ...
        if (completedFiles.length === 1) {
            ...
            const fileUrl = completedFiles[0].url;
            linkToCopy = fileUrl.startsWith('http') ? fileUrl.replace('/get', '') + '?dl=1' : window.location.origin + '/get' + fileUrl;
        } else {
            // Multiple files - batch ZIP link
            const fileTokens = completedFiles.map(f => {
                // Extract token from URL (format: /{token}/{filename} or full URL)
                let url = f.url;
                ...
                // urlParts = ['', 'token', 'filename'] so we want token/filename
                return `${urlParts[1]}/${encodeURIComponent(f.name)}`;
            });
            
            const zipUrl = `/(${fileTokens.join(',')}).zip`;
            linkToCopy = window.location.origin + zipUrl;
        }
    ...
    function updateBatchActions() {
        ...
        if (downloadAllBtn) {
            ...
            downloadAllBtn.textContent = completedFiles.length > 1 ? 'Copy ZIP link' : 'Copy link';
        }
